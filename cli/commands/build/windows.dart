...
class WindowsBuildCommand extends Command with BuildCommandCommonSteps {
  ...
  void run() async {
    ...
    final chocoFiles = [
      ...
      join(cwd.path, "choco-struct", "kelletube.nuspec"),
    ];
    ...
    final ogExe = File(
      join(
        ...
        "kelletube-${pubspec.version}-windows-setup.exe",
      ),
    );

    final exePath = join(cwd.path, "dist", "Kelletube-windows-x86_64-setup.exe");
    ...
    final chocoNupkg = File(
      join(cwd.path, "dist", "kelletube.$versionWithoutBuildNumber.nupkg"),
    );

    final distNupkgPath = join(
      cwd.path,
      "dist",
      "Kelletube-windows-x86_64.nupkg",
    );
