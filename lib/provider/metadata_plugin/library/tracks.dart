...
class MetadataPluginSavedTracksNotifier
    extends AutoDisposePaginatedAsyncNotifier<KelletubeFullTrackObject> {
  ...
  Future<void> addFavorite(List<KelletubeTrackObject> tracks) async {
    if (state.value == null) {
      return;
    }
    ...
    state = AsyncData(
      state.value!.copyWith(
        items: [
          ...tracks.whereType<KelletubeFullTrackObject>(),
          ...state.value!.items
        ],
      ),
    );
  ...
  Future<void> removeFavorite(List<KelletubeTrackObject> tracks) async {
    if (state.value == null) {
      return;
    }
