...
import 'package:kelletube/provider/metadata_plugin/core/scrobble.dart';
...
import 'package:kelletube/provider/skip_segments/skip_segments.dart';
...
import 'package:kelletube/services/audio_services/audio_services.dart';
...
class AudioPlayerStreamListeners {
  ...
  StreamSubscription subscribeToScrobbleChanged() {
    ...
    return audioPlayer.positionStream.listen((position) async {
      try {
        final uid = audioPlayerState.activeTrack is KelletubeLocalTrackObject
            ? (audioPlayerState.activeTrack as KelletubeLocalTrackObject).path
        ...
        if (activeTrack.artists.any((a) => a.images == null)) {
          ...
          activeTrack = activeTrack.copyWith(
            artists: artists
                .map((e) => KelletubeSimpleArtistObject.fromJson(e.toJson()))
                .toList(),
          );
        }
    ...
    return audioPlayer.positionStream.listen((event) async {
      ...
      try {
        ...
        if (nextTrack == null ||
            ...
            nextTrack is KelletubeLocalTrackObject) {
          return;
        }

        try {
          await ref.read(
            sourcedTrackProvider(nextTrack as KelletubeFullTrackObject).future,
          );
        } finally {
          lastTrack = nextTrack.id;
        }
